...
class PipelineConfigurationTests(unittest.TestCase):
    ...
    def test_resolve_all_iso3_country_codes_multi_country(self):
        ...
        step_data = {
            "pbf_references": [
                {"name": "Dänemark", "filename": "denmark-latest.osm.pbf", "osm_id": "denmark"},
                {"name": "Niedersachsen", "filename": "niedersachsen-latest.osm.pbf", "osm_id": "germany/niedersachsen"},
            ]
        }

        codes = resolve_all_iso3_country_codes(location=loc, step_data=step_data)
        self.assertEqual(codes, "DEU, DNK")
