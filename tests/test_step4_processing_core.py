...
class Step4ProcessingCoreTests(unittest.TestCase):
    ...
    def test_nationwide_and_model_params_persistence(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(nw_file, "w", encoding="utf-8") as f:
                json.dump({"enabled": True, "radius": 600, "min_intensity": 8, "min_intensity_level_3": 12}, f)
            with open(m2_file, "w", encoding="utf-8") as f:
                json.dump({"min_weight": 3}, f)
            with open(m5_file, "w", encoding="utf-8") as f:
                json.dump({"buffer_m": 450}, f)

            with open(nw_file, "r", encoding="utf-8") as f:
                nw_data = json.load(f)
            self.assertTrue(nw_data["enabled"])
            self.assertEqual(nw_data["radius"], 600)
            self.assertEqual(nw_data["min_intensity"], 8)
            self.assertEqual(nw_data["min_intensity_level_3"], 12)
