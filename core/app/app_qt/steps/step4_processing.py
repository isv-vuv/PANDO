...
class PipelineWorker(_Q_OBJECT_BASE):
    ...
    def stop(self) -> None:
        self.stop_event.set()
        self.log_ready.emit(localizer.get_string("step5_status_cancel_requested", default="Abbruch angefordert …"))
...
class Step4ProcessingWidget(_Q_WIDGET_BASE):
    ...
    def __init__(self, localizer, step4_data: dict, parent: Optional[object] = None):
        ...
        self._current_phase = self.localizer.get_string("step5_phase_preparation", default="Vorbereitung")
    ...
    def _build_ui(self) -> None:
        ...
        header, _title_label, _step_label = create_step_header(
            self.localizer.get_string("step5_title", default="Schritt 4: Parameterprüfung und Verarbeitung"),
            current_step=4,
            total_steps=6,
        ...
        readiness_box = QtWidgets.QGroupBox(escape_mnemonic(self.localizer.get_string("step5_group_derived_models")), content)
        ...
        info_row = QtWidgets.QLabel(
            f"<b>{self.localizer.get_string('step5_label_project')}:</b> {self.project_path or '–'} &nbsp;&nbsp;·&nbsp;&nbsp; "
            f"<b>{self.localizer.get_string('step5_label_local_crs')}:</b> {crs or '–'}<br>"
            f"<b>{self.localizer.get_string('step5_label_cell_size')}:</b> {self.step_data.get('cell_size_m', 4500)} m &nbsp;&nbsp;·&nbsp;&nbsp; "
            f"<b>{self.localizer.get_string('step5_label_min_extent')}:</b> {self.step_data.get('radius_km', 30)} km",
            readiness_box,
        )
        ...
        external_box = QtWidgets.QGroupBox(escape_mnemonic(self.localizer.get_string("step5_group_external_data")), content)
        ...
        lbl_ask_ref = QtWidgets.QLabel(self.localizer.get_string("step5_ask_use_reference", default="Lokale Referenzdaten verwenden:"), external_box)
        ...
        self.lbl_ref = self._create_parameter_label("step5_label", "reference_layer", "Bevölkerungs-Polygonlayer", self.reference_edit, external_box)
        ...
        self.lbl_pop = self._create_parameter_label("step5_label", "population_field", "Feld mit Bevölkerungszahl", self.reference_field_edit, external_box)
        ...
        lbl_ask_census = QtWidgets.QLabel(self.localizer.get_string("step5_ask_use_census", default="Alternatives Zensusraster verwenden:"), external_box)
        ...
        self.lbl_census = self._create_parameter_label("step5_label", "census_grid", "Alternatives Zensusraster", self.census_edit, external_box)
        ...
        model2_box = QtWidgets.QGroupBox(escape_mnemonic(self.localizer.get_string("step5_group_model2")), models_widget)
        ...
        model5_box = QtWidgets.QGroupBox(escape_mnemonic(self.localizer.get_string("step5_group_model5")), models_widget)
        ...
        execution_box = QtWidgets.QGroupBox(escape_mnemonic(self.localizer.get_string("step5_group_execution")), content)
        ...
        info_label = QtWidgets.QLabel(self.localizer.get_string("step5_info_start_models"), execution_box)
        ...
        self.phase_label = QtWidgets.QLabel(self.localizer.get_string("step5_label_ready"), execution_box)
        self.elapsed_label = QtWidgets.QLabel("00:00", execution_box)
        self.overall_progress = QtWidgets.QProgressBar(execution_box)
        self.phase_progress = QtWidgets.QProgressBar(execution_box)
        self.start_button = QtWidgets.QPushButton(self.localizer.get_string("step5_button_start"), execution_box)
        self.restart_button = QtWidgets.QPushButton(self.localizer.get_string("step5_button_restart_models"), execution_box)
        self.restart_button.setToolTip(
            self.localizer.get_string(
                "step5_tooltip_restart",
        ...
        self.stop_button = QtWidgets.QPushButton(self.localizer.get_string("step5_button_stop"), execution_box)
        self.stop_button.setEnabled(False)
        self.start_button.clicked.connect(self.start_pipeline)
        self.restart_button.clicked.connect(self.restart_pipeline)
        self.stop_button.clicked.connect(self.stop_pipeline)

        lbl_step = QtWidgets.QLabel(self.localizer.get_string("step5_label_current_step"), execution_box)
        ...
        lbl_phase_prog = QtWidgets.QLabel(self.localizer.get_string("step5_label_step_progress"), execution_box)
        execution.addWidget(lbl_phase_prog, 2, 0)
        execution.addWidget(self.phase_progress, 2, 1, 1, 2)

        lbl_overall_prog = QtWidgets.QLabel(self.localizer.get_string("step5_label_overall_progress"), execution_box)
        ...
        log_group = QtWidgets.QGroupBox(self.localizer.get_string("step5_group_log"), content)
    ...
    def _handle_next_clicked(self) -> None:
        if not self.is_pipeline_completed():
            Dialogs.warning(
                self,
                self.localizer.get_string("step5_warning_models_incomplete_title"),
                self.localizer.get_string("step5_warning_models_incomplete_body"),
            )
            return
    ...
    def refresh_readiness(self) -> None:
        ...
        if self.rb_ref_yes.isChecked():
            ...
            if not reference:
                blockers.append(self.localizer.get_string("step5_blocker_select_ref"))
            elif not os.path.isfile(reference):
                blockers.append(self.localizer.get_string("step5_blocker_ref_missing", reference=reference))
            if not self.reference_field_edit.text().strip():
                blockers.append(self.localizer.get_string("step5_blocker_pop_missing"))
        if self.rb_census_yes.isChecked():
            census = self.census_edit.text().strip()
            if not census:
                blockers.append(self.localizer.get_string("step5_blocker_census_missing", census=""))
            elif not os.path.isfile(census):
                blockers.append(self.localizer.get_string("step5_blocker_census_missing", census=census))
        if blockers:
            prefix = self.localizer.get_string("step5_status_not_ready", default="Nicht bereit:")
            self.readiness_label.setText(f"{prefix}\n• " + "\n• ".join(blockers))
        else:
            self.readiness_label.setText(self.localizer.get_string("step5_status_all_ready", default="Alle Input-Daten bereit"))
        if self.start_button is not None:
            btn_key = "step5_button_resume" if getattr(self, "_stopped_by_user", False) else "step5_button_start"
    ...
    def restart_pipeline(self) -> None:
        if not Dialogs.confirm(
            self,
            self.localizer.get_string("step5_dialog_confirm_restart_title", default="Alle Modelle neu berechnen"),
            self.localizer.get_string("step5_dialog_confirm_restart_body", default="Sollen Modell 1 bis Modell 6 und OSM Phase C wirklich neu berechnet werden?"),
        ):
            return
    ...
    def start_preprocessing(self) -> None:
        ...
        if basic_blockers:
            self.log_box.appendPlainText(self.localizer.get_string("step5_log_phase_a_waiting", blockers="; ".join(basic_blockers), default="OSM-Phase A wartet: " + "; ".join(basic_blockers)))
            return
        ...
        if reusable is not None:
            ...
            self.phase_progress.setFormat(self.localizer.get_string("step5_format_phase_a_ready", default="%p% – OSM-Vorverarbeitung vorhanden"))
            self.phase_label.setText(self.localizer.get_string("step5_status_phase_a_ready", default="OSM-Vorverarbeitung bereits vorhanden"))
            self.log_box.appendPlainText(
                self.localizer.get_string("step5_log_phase_a_reused", default="Vorhandene OSM-Phase-A-Dateien werden wiederverwendet; keine Neuberechnung nötig.")
            )
            for key, path in reusable.items():
                self.log_box.appendPlainText(f"{key}: {path}")
            self.status_changed.emit(self.localizer.get_string("step5_status_phase_a_ready", default="OSM-Vorverarbeitung bereits vorhanden"))
            self.refresh_readiness()
            return
    ...
    def _handle_finished(self, _context: object) -> None:
        ...
        if self._phase_a_only:
            self.phase_label.setText(self.localizer.get_string("step5_status_phase_a_done", default="OSM-Vorverarbeitung abgeschlossen"))
            self.log_box.appendPlainText(self.localizer.get_string("step5_log_phase_a_done", default="OSM-Phase A abgeschlossen; Parameter können freigegeben werden."))
            self.status_changed.emit(self.localizer.get_string("step5_status_phase_a_done", default="OSM-Vorverarbeitung abgeschlossen"))
        else:
            self._is_completed = True
            self._stopped_by_user = False
            self.overall_progress.setValue(100)
            self.phase_label.setText(self.localizer.get_string("step5_status_completed", default="Abgeschlossen"))
            self.log_box.appendPlainText(self.localizer.get_string("step5_log_pipeline_completed", default="Pipeline vollständig abgeschlossen."))
            self.status_changed.emit(self.localizer.get_string("step5_status_pipeline_done", default="Pipeline abgeschlossen"))
            self.refresh_readiness()

    def _handle_error(self, phase: str, message: str) -> None:
        self.phase_label.setText(self.localizer.get_string("step5_status_error_phase", phase=phase, default=f"Fehler: {phase}"))
        self.log_box.appendPlainText(self.localizer.get_string("step5_log_error_phase", phase=phase, message=message, default=f"FEHLER [{phase}]: {message}"))
        self.status_changed.emit(self.localizer.get_string("step5_status_error_phase", phase=phase, default=f"{phase} fehlgeschlagen"))
        Dialogs.error(self, self.localizer.get_string("step5_status_error_phase", phase=phase, default=f"Fehler in {phase}"), message)
    ...
    def _browse_into(self, target, file_filter: str) -> None:
        path = Dialogs.open_file(self, self.localizer.get_string("step5_dialog_select_file", default="Datei auswählen"), file_filter, self.project_path)
        if path:
            target.setText(path)
