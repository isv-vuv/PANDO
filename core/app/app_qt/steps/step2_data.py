...
class Step4MapPreview(_Q_WIDGET_BASE):
    ...
    def update_preview(self, jobs: list[dict], radius_km: int) -> None:
        ...
        if isinstance(self.location, dict):
            lat = self.location.get("latitude") if self.location.get("latitude") is not None else self.location.get("lat")
            lon = self.location.get("longitude") if self.location.get("longitude") is not None else self.location.get("lon")
        else:
            lat = getattr(self.location, "latitude", None)
            if lat is None:
                lat = getattr(self.location, "lat", None)
            lon = getattr(self.location, "longitude", None)
            if lon is None:
                lon = getattr(self.location, "lon", None)
...
class PbfSearchWorker(_Q_OBJECT_BASE):
    ...
    regions_found = pyqtSignal(object) if pyqtSignal is not None else None
    size_updated = pyqtSignal(str, object) if pyqtSignal is not None else None
    ...
    def run(self) -> None:
        ...
        def on_regions_found(details: dict) -> None:
            if self.regions_found is not None and not self._cancel_event.is_set():
                self.regions_found.emit(details)

        def on_size_updated(osm_id: str, size_bytes: int) -> None:
            if self.size_updated is not None and not self._cancel_event.is_set():
                self.size_updated.emit(str(osm_id), size_bytes)
        ...
        try:
            details = find_pbf_details(
                ...
                log=log_adapter,
                on_regions_found=on_regions_found,
                on_size_updated=on_size_updated,
            )
...
class Step2DataWidget(_Q_WIDGET_BASE):
    ...
    def _build_ui(self) -> None:
        ...
        # Clean Info Row (frameless): Location & Project path on left, "Projektordner öffnen" button on right
        info_widget = QtWidgets.QWidget(self)
        info_layout = QtWidgets.QHBoxLayout(info_widget)
        info_layout.setContentsMargins(4, 2, 4, 6)
        info_layout.setSpacing(12)

        info_text_widget = QtWidgets.QWidget(info_widget)
        info_text_layout = QtWidgets.QVBoxLayout(info_text_widget)
        ...
        if location_name:
            ...
            location_label.setFont(app_font(10))
            location_label.setStyleSheet("color: #475569;")
            location_label.setWordWrap(True)
            info_text_layout.addWidget(location_label)

        self.project_label = QtWidgets.QLabel(self.localizer.get_string("step2_label_project_dir_not_set"), info_text_widget)
        self.project_label.setFont(app_font(10, qfont_bold()))
        self.project_label.setStyleSheet("color: #1e293b;")
        self.project_label.setWordWrap(True)
        info_text_layout.addWidget(self.project_label)
        ...
        self.open_project_button = QtWidgets.QPushButton(self.localizer.get_string("step2_button_open_folder"), info_widget)
        self.open_project_button.clicked.connect(self.open_project_folder)
        info_layout.addWidget(self.open_project_button, 0, qt_enum(Qt, "AlignVCenter", "AlignmentFlag"))
        ...
        self.global_toggle_btn = QtWidgets.QToolButton(self.global_box)
        self.global_toggle_btn.setText(self.localizer.get_string("step2_accordion_global"))
        self.global_toggle_btn.setToolButtonStyle(qt_enum(Qt, "ToolButtonTextBesideIcon", "ToolButtonStyle"))
        self.global_toggle_btn.setArrowType(qt_enum(Qt, "DownArrow", "ArrowType"))
        self.global_toggle_btn.setFont(app_font(10, qfont_bold()))
        self.global_toggle_btn.setCursor(qt_enum(Qt, "PointingHandCursor", "CursorShape"))
        ...
        self.pbf_toggle_btn = QtWidgets.QToolButton(self.pbf_box)
        self.pbf_toggle_btn.setText(self.localizer.get_string("step2_accordion_osm"))
        self.pbf_toggle_btn.setToolButtonStyle(qt_enum(Qt, "ToolButtonTextBesideIcon", "ToolButtonStyle"))
        self.pbf_toggle_btn.setArrowType(qt_enum(Qt, "DownArrow", "ArrowType"))
        self.pbf_toggle_btn.setFont(app_font(10, qfont_bold()))
        self.pbf_toggle_btn.setCursor(qt_enum(Qt, "PointingHandCursor", "CursorShape"))
        self.pbf_toggle_btn.setStyleSheet(
            "QToolButton { border: none; background: transparent; color: #1e293b; padding: 4px 2px; } "
            "QToolButton:hover { color: #2563eb; }"
        )
    ...
    def _set_accordion_collapsed(self, container, toggle_btn, locale_key) -> None:
        """Collapse one accordion section to zero height and update its button text and arrow."""
        container.setMaximumHeight(0)
        container.setVisible(False)
        toggle_btn.setArrowType(qt_enum(Qt, "RightArrow", "ArrowType"))
        toggle_btn.setText(self.localizer.get_string(locale_key))
    ...
    def toggle_global_box(self) -> None:
        ...
        if visible:
            ...
            self.global_toggle_btn.setArrowType(qt_enum(Qt, "RightArrow", "ArrowType"))
        self.global_toggle_btn.setText(self.localizer.get_string("step2_accordion_global"))
    ...
    def toggle_pbf_box(self) -> None:
        ...
        if visible:
            ...
            self.pbf_toggle_btn.setArrowType(qt_enum(Qt, "RightArrow", "ArrowType"))
        self.pbf_toggle_btn.setText(self.localizer.get_string("step2_accordion_osm"))
        self.global_box.adjustSize()
        self.pbf_box.adjustSize()
    ...
    def start_pbf_search(self, force: bool = False) -> None:
        ...
        if not force:
            ...
            if cached and "pbfs" in cached:
                ...
                self._sync_download_jobs_sizes()
                self._update_map_preview()
        ...
        self._search_worker.regions_found.connect(self._handle_pbf_regions_found)
        self._search_worker.size_updated.connect(self._handle_pbf_size_updated)
    ...
    def cancel_all_workers(self) -> None:
        ...
        if self._search_worker is not None:
            try:
                self._search_worker.cancel()
                if self._search_worker.regions_found:
                    self._search_worker.regions_found.disconnect()
                if self._search_worker.size_updated:
                    self._search_worker.size_updated.disconnect()
        ...
        if self._search_thread is not None:
            try:
                ...
                if getattr(self._search_thread, "isRunning", lambda: False)():
                    self._search_thread.requestInterruption()
                    self._search_thread.setParent(None)
            except Exception:
                pass
            self._search_thread = None
            self._search_worker = None
        ...
        if self._verify_thread is not None:
            try:
                ...
                self._verify_thread.wait(100)
                if getattr(self._verify_thread, "isRunning", lambda: False)():
                    self._verify_thread.requestInterruption()
                    self._verify_thread.setParent(None)
            except Exception:
                pass
            self._verify_thread = None
            self._verify_worker = None
        ...
        if self._download_thread is not None:
            try:
                ...
                self._download_thread.wait(100)
                if getattr(self._download_thread, "isRunning", lambda: False)():
        ...
        if self._global_download_thread is not None:
            try:
                ...
                self._global_download_thread.wait(100)
                if getattr(self._global_download_thread, "isRunning", lambda: False)():
                    self._global_download_thread.requestInterruption()
                    self._global_download_thread.setParent(None)
            except Exception:
                pass
            self._global_download_thread = None
            self._global_download_worker = None
        ...
        if self._index_update_thread is not None:
            try:
                ...
                self._index_update_thread.wait(100)
                if getattr(self._index_update_thread, "isRunning", lambda: False)():
                    self._index_update_thread.requestInterruption()
                    self._index_update_thread.setParent(None)
            except Exception:
                pass
            self._index_update_thread = None
            self._index_update_worker = None
    ...
    def _sync_download_jobs_sizes(self) -> None:
        if not self.download_jobs:
            return
        try:
            size_map = {}

            # 1. Collect sizes from global persistent cache
            try:
                from core.app.app_core.geofabrik import _URL_SIZE_CACHE
                for u, sz in _URL_SIZE_CACHE.items():
                    if sz and sz > 0:
                        fn_u = os.path.basename(u).lower()
                        norm_u = normalize_pbf_name(fn_u)
                        size_map[u.lower()] = sz
                        size_map[fn_u] = sz
                        size_map[norm_u] = sz
            except Exception:
            ...
            # 2. Collect sizes from pbf_details
            if self.pbf_details:
                def _collect_sizes(regions):
                    for r in regions:
                        if not isinstance(r, dict):
                            continue
                        try:
                            sb = int(r.get("size_bytes") or r.get("bytes") or 0)
                        except (TypeError, ValueError):
                            sb = 0
                        if sb > 0:
                            if r.get("osm_id"):
                                size_map[str(r["osm_id"]).lower()] = sb
                            if r.get("id"):
                                size_map[str(r["id"]).lower()] = sb
                            if r.get("filename"):
                                size_map[str(r["filename"]).lower()] = sb
                                size_map[normalize_pbf_name(str(r["filename"]))] = sb
                            if r.get("pbf_url"):
                                size_map[str(r["pbf_url"]).lower()] = sb
                        if "sub_regions" in r and isinstance(r["sub_regions"], list):
                            _collect_sizes(r["sub_regions"])

                _collect_sizes(self.pbf_details.get("pbfs", []))

            # 3. Check local disk if file is already present
            candidate_dirs = []
            if self.data_path:
                candidate_dirs.append(self.data_path)
            if self.project_path:
                candidate_dirs.append(os.path.join(self.project_path, "input"))
                candidate_dirs.append(os.path.join(self.project_path, "01_data"))
                candidate_dirs.append(os.path.join(self.project_path, "processed", "osm", "01_input"))
            candidate_dirs.append(os.path.join(tool_root(), "core", "data", "osm"))
            ...
            for job in self.download_jobs:
                if not isinstance(job, dict):
                    continue
                try:
                    curr_size = int(job.get("size_bytes") or job.get("bytes") or 0)
                except (TypeError, ValueError):
                    curr_size = 0
                if curr_size <= 0:
                    fn = str(job.get("filename", "")).lower()
                    oid = str(job.get("osm_id", "")).lower()
                    jid = str(job.get("id", "")).lower()
                    url_j = str(job.get("url", "")).lower()
                    norm = normalize_pbf_name(fn)
                    matched_size = size_map.get(url_j) or size_map.get(oid) or size_map.get(jid) or size_map.get(fn) or size_map.get(norm)
                    if not matched_size and fn:
                        for pbf_dir in candidate_dirs:
                            if pbf_dir and os.path.isdir(pbf_dir):
                                try:
                                    for f in os.listdir(pbf_dir):
                                        if f.lower().startswith(norm) and f.lower().endswith(".osm.pbf"):
                                            fp = os.path.join(pbf_dir, f)
                                            if os.path.isfile(fp):
                                                matched_size = os.path.getsize(fp)
                                                break
                                except Exception:
                                    pass
                            if matched_size:
                    ...
                    if matched_size and matched_size > 0:
                        job["size_bytes"] = matched_size
                        job["bytes"] = matched_size
                        changed = True
    ...
    def _handle_pbf_regions_found(self, details: dict) -> None:
        """Immediately displays candidate regions with pending/cached sizes."""
    ...
    def _handle_pbf_size_updated(self, osm_id: str, size_bytes: int) -> None:
        """Progressively updates the size of a specific region row in available/selected trees."""
        if self._search_cancelled or not osm_id:
            return

        def _update_node_size(item):
            data = item.data(0, qt_enum(Qt, "UserRole", "ItemDataRole"))
            if isinstance(data, dict) and (
                str(data.get("osm_id")) == osm_id
                or str(data.get("id")) == osm_id
                or str(data.get("filename")) == osm_id
            ):
                data["size_bytes"] = size_bytes
                item.setText(1, format_gb(size_bytes))
            for child_idx in range(item.childCount()):
                _update_node_size(item.child(child_idx))

        # 1. Update available_tree items recursively
        root = self.available_tree.invisibleRootItem()
        for i in range(root.childCount()):
            _update_node_size(root.child(i))

        # 2. Update selected_tree item if region was already selected
        if hasattr(self, "selected_tree"):
            sel_root = self.selected_tree.invisibleRootItem()
            size_changed = False
            for k in range(sel_root.childCount()):
                sel_item = sel_root.child(k)
                sel_data = sel_item.data(0, qt_enum(Qt, "UserRole", "ItemDataRole"))
                if isinstance(sel_data, dict) and (
                    str(sel_data.get("osm_id")) == osm_id
                    or str(sel_data.get("id")) == osm_id
                    or str(sel_data.get("filename")) == osm_id
                ):
                    sel_data["size_bytes"] = size_bytes
                    sel_item.setText(1, format_gb(size_bytes))
                    size_changed = True
            if size_changed:
                self._update_total_size()
    ...
    def _refresh_available_tree(self) -> None:
        ...
        def _add_node(parent_target, region_dict: dict, indent: int = 0):
            prefix = "  " * indent
            name = prefix + region_dict.get("name", region_dict.get("filename", ""))
            item = QtWidgets.QTreeWidgetItem(
                [
                    name,
                    format_gb(region_dict.get("size_bytes", 0)),
                ]
            )
            item.setTextAlignment(1, 0x0002 | 0x0080)  # AlignRight | AlignVCenter
            item.setData(0, qt_enum(Qt, "UserRole", "ItemDataRole"), region_dict)
            if isinstance(parent_target, QtWidgets.QTreeWidget):
                parent_target.addTopLevelItem(item)
            else:
                parent_target.addChild(item)
            for sub in region_dict.get("sub_regions", []):
                _add_node(item, sub, indent + 1)
            item.setExpanded(True)
            return item

        for pbf in pbfs:
            _add_node(self.available_tree, pbf, 0)
    ...
    def _start_verify_worker(self, show_feedback: bool = False, confirm_after: bool = False) -> None:
        ...
        self._verify_thread = QtCore.QThread()
    ...
    def check_global_datasets_status(self, show_feedback: bool = False) -> None:
        ...
        if not missing:
            msg = self.localizer.get_string(
                ...
                default="Alle 5 globalen Datensätze vorhanden (GHS-POP und GADM ADM0-3)."
            )
            ...
            if self.global_toggle_btn:
                self.global_toggle_btn.setArrowType(qt_enum(Qt, "RightArrow", "ArrowType"))
                self.global_toggle_btn.setText(self.localizer.get_string('step2_accordion_global_full', default='Globale Daten (GHS, GADM) [5/5 vorhanden]'))
            ...
            if self.pbf_toggle_btn:
                self.pbf_toggle_btn.setArrowType(qt_enum(Qt, "DownArrow", "ArrowType"))
                self.pbf_toggle_btn.setText(self.localizer.get_string('step2_accordion_osm', default='OSM-Daten'))
            if show_feedback:
                Dialogs.info(self, self.localizer.get_string("step2_dialog_global_status_title", default="GHS-POP & GADM Status"), msg)
        else:
            ...
            if self.global_toggle_btn:
                self.global_toggle_btn.setArrowType(qt_enum(Qt, "DownArrow", "ArrowType"))
                self.global_toggle_btn.setText(self.localizer.get_string('step2_accordion_global_partial', present_count=present_count, default=f'Globale Daten (GHS, GADM) [{present_count}/5 vorhanden]'))
            if self.global_container:
                self.global_container.show()
            if show_feedback:
                Dialogs.warning(self, self.localizer.get_string("step2_dialog_global_status_title", default="GHS-POP & GADM Status"), msg)
        self._update_next_button_state()
    ...
    def start_global_download(self) -> None:
        ...
        self._global_download_thread = QtCore.QThread()
    ...
    def update_geofabrik_index(self) -> None:
        ...
        self._index_update_thread = QtCore.QThread()
