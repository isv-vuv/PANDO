...
from core.app.app_core.geofabrik import find_pbf_details, download_geofabrik_index, get_geofabrik_index_info
...
from core.app.app_qt.qt_base import Dialogs, Qt, QtCore, QtGui, QtWidgets, app_font, create_step_header, escape_mnemonic, pyqtSignal, qfont_bold, qt_enum, require_qgis_qt
...
try:
    ...
    QGIS_STEP4_MAP_IMPORT_ERROR = None
    ...
    QGIS_STEP4_MAP_IMPORT_ERROR = exc
...
class Step4MapPreview(_Q_WIDGET_BASE):
    ...
    def __init__(self, localizer, location, geofabrik_index: dict, parent=None):
        if QGIS_STEP4_MAP_IMPORT_ERROR is not None:
            raise RuntimeError(f"QGIS step 4 map imports failed: {QGIS_STEP4_MAP_IMPORT_ERROR}")
        super().__init__(parent)
...
class GlobalDataDownloadWorker(_Q_OBJECT_BASE):
    ...
    def run(self) -> None:
            ...
            if self.progress_updated:
                if total > 0:
                    ...
                    msg = localizer.get_string(
                        "step4_status_downloading_file_progress",
                        filename=filename,
        ...
        try:
            ...
            if self.finished:
                if self._cancelled:
                    self.finished.emit(False, localizer.get_string("step4_status_download_cancelled", default="Download abgebrochen."))
                else:
                    self.finished.emit(True, localizer.get_string("step4_status_global_data_downloaded", default="Alle POP & GADM Daten erfolgreich heruntergeladen."))
        except Exception as exc:
            if self.finished:
...
class Step3DataWidget(_Q_WIDGET_BASE):
    ...
    def _build_ui(self) -> None:
        ...
        header, _title_label, _step_label = create_step_header(
            self.localizer.get_string("step4_title", default="Schritt 3: Geodaten"),
            current_step=3,
            total_steps=6,
        ...
        dir_box = QtWidgets.QGroupBox(self.localizer.get_string("step4_header_project_info", default="Projekt-Informationen"), self)
        ...
        if location_name:
            location_label = QtWidgets.QLabel(
                self.localizer.get_string("step4_location_info", location_name=location_name),
                dir_box,
            )
            location_label.setWordWrap(True)
            dir_layout.addWidget(location_label, 0, 0, 1, 2)

        self.create_project_button = QtWidgets.QPushButton(self.localizer.get_string("step4_button_create_project_dir"))
        self.create_project_button.hide()

        self.open_project_button = QtWidgets.QPushButton(self.localizer.get_string("step4_button_open_folder"), dir_box)
        self.open_project_button.clicked.connect(self.open_project_folder)
        dir_layout.addWidget(self.open_project_button, 1, 0)

        self.project_label = QtWidgets.QLabel(self.localizer.get_string("step4_label_project_dir_not_set"), dir_box)
        ...
        self.global_toggle_btn = QtWidgets.QPushButton(
            escape_mnemonic("▼ " + self.localizer.get_string("step4_accordion_global")), self.global_box
        )
        ...
        self.global_status_label = QtWidgets.QLabel(
            self.localizer.get_string("step4_status_checking_datasets"),
        ...
        self.global_download_button = QtWidgets.QPushButton(
            self.localizer.get_string("step4_button_download", default="Herunterladen"),
            self.global_container,
        )
        ...
        self.global_browser_button = QtWidgets.QPushButton(
            self.localizer.get_string("step4_button_open_browser", default="Im Browser öffnen"),
            self.global_container,
        )
        ...
        self.global_check_button = QtWidgets.QPushButton(
            self.localizer.get_string("step4_button_check_status", default="Status prüfen"),
            self.global_container,
        )
        ...
        self.global_file_progress_bar = QtWidgets.QProgressBar(self.global_container)
        ...
        self.global_progress_bar = QtWidgets.QProgressBar(self.global_container)
        ...
        self.pbf_toggle_btn = QtWidgets.QPushButton(
            escape_mnemonic("▼ " + self.localizer.get_string("step4_accordion_osm")), self.pbf_box
        )
        ...
        self.index_status_label = QtWidgets.QLabel(
            self.localizer.get_string("step4_status_checking_geofabrik"),
            self.pbf_container,
        )
        index_row.addWidget(self.index_status_label)
        self.index_update_button = QtWidgets.QPushButton(
            self.localizer.get_string("step4_button_update_index", default="Index aktualisieren"),
            self.pbf_container,
        )
        ...
        radius_label = QtWidgets.QLabel(self.localizer.get_string("step4_label_pbf_search_radius"), self.pbf_container)
        ...
        self.search_button.clicked.connect(self.start_pbf_search)
        ...
        av_label = QtWidgets.QLabel(self.localizer.get_string("step4_label_available_regions"), available_panel)
        ...
        self.search_spinner_label = QtWidgets.QLabel(
            self.localizer.get_string("step4_status_searching_pbf"),
            available_panel,
        )
        ...
        self.available_tree.setHeaderLabels(
            [
                self.localizer.get_string("step4_label_column_region"),
                self.localizer.get_string("step4_label_column_size"),
            ]
        )
        ...
        self.clear_button.setToolTip(self.localizer.get_string("step4_button_clear_list"))
        ...
        sel_label = QtWidgets.QLabel(self.localizer.get_string("step4_label_selection_for_download"), selected_panel)
        ...
        self.selected_tree.setHeaderLabels(
            [
                self.localizer.get_string("step4_label_column_region"),
                self.localizer.get_string("step4_label_column_size"),
                self.localizer.get_string("step4_label_column_availability"),
            ]
        )
        ...
        self.pbf_file_progress_bar = QtWidgets.QProgressBar(left_pbf_panel)
        ...
        self.progress_bar = QtWidgets.QProgressBar(left_pbf_panel)
        ...
        map_title = QtWidgets.QLabel(self.localizer.get_string("step4_map_title"), right_map_panel)
    ...
    def toggle_global_box(self) -> None:
        ...
        self.global_toggle_btn.setText(escape_mnemonic(("\u25bc " if visible else "\u25b6 ") + self.localizer.get_string("step4_accordion_global")))
        if visible:
            self._set_accordion_collapsed(self.pbf_container, self.pbf_toggle_btn, "step4_accordion_osm")
    ...
    def toggle_pbf_box(self) -> None:
        ...
        self.pbf_toggle_btn.setText(escape_mnemonic(("\u25bc " if visible else "\u25b6 ") + self.localizer.get_string("step4_accordion_osm")))
        if visible:
            self._set_accordion_collapsed(self.global_container, self.global_toggle_btn, "step4_accordion_global")
        self.global_box.adjustSize()
        self.pbf_box.adjustSize()
    ...
    def create_project_directory(self) -> None:
        Dialogs.warning(
            ...
            self.localizer.get_string("step4_error_project_dir_first"),
        )
    ...
    def _set_project_ready(self, ready: bool) -> None:
        ...
        if ready and self.project_path:
            self.project_label.setText(
                self.localizer.get_string("step4_label_project_dir_set", path=os.path.normpath(self.project_path))
            )
        else:
            self.project_label.setText(self.localizer.get_string("step4_label_project_dir_not_set"))



    def start_pbf_search(self) -> None:
        ...
        try:
            ...
            Dialogs.error(
                self,
                self.localizer.get_string("step4_error_invalid_radius_title"),
                self.localizer.get_string("step4_error_invalid_radius_message"),
            )
            return
        ...
        self._set_busy(True)
        self._emit_status(self.localizer.get_string("step4_status_searching_pbf"))

        self._search_thread = QtCore.QThread(self)
        self._search_worker = PbfSearchWorker(self.selected_location, radius_km, self.geofabrik_index)
    ...
    def _handle_pbf_search_error(self, message: str) -> None:
        if self._search_cancelled:
            return
        self._refresh_available_tree()
        self._emit_status(self.localizer.get_string("step4_status_project_dir_set"))
        self._emit_status(self.localizer.get_string("step4_status_pbf_search_error", error=message))
    ...
    def start_download(self) -> None:
        if not self.download_jobs:
            Dialogs.warning(
                self,
                self.localizer.get_string("step4_error_no_download_title"),
                self.localizer.get_string("step4_error_container_no_download_message"),
            )
            return
        ...
        self._emit_status(self.localizer.get_string("step4_status_preparing_download"))
    ...
    def stop_download(self) -> None:
        if self._download_worker:
            self._download_worker.stop()
            self._emit_status(self.localizer.get_string("status_stopping_download"))
    ...
    def _handle_download_finished(self, success: bool) -> None:
        ...
        if success:
            ...
            self.pbf_file_progress_bar.setValue(100)
            self.pbf_file_progress_label.setText("")
            self.pbf_overall_progress_label.setText(self.localizer.get_string("step4_status_all_downloads_complete", default="Download abgeschlossen."))
            self.check_pbf_files(show_feedback=False, force=True)
        else:
            self._emit_status(self.localizer.get_string("step4_status_download_interrupted"))
    ...
    def _update_total_size(self) -> None:
        ...
        total_label = self.localizer.get_string("step4_label_total_size")
    ...
    def _availability_label_text(self, state: str) -> str:
        key_by_state = {
            "offline": "step4_availability_offline",
            "stale": "step4_availability_stale",
            "download": "step4_availability_download",
            "downloading": "step4_availability_downloading",
            "checking": "step4_availability_checking",
        }
        return self.localizer.get_string(key_by_state.get(state, "step4_availability_checking"))
    ...
    def _start_verify_worker(self, show_feedback: bool = False, confirm_after: bool = False) -> None:
        ...
        self._verify_restart_requested = False
        self._set_busy(True)
        self._emit_status(self.localizer.get_string("step4_status_verifying_files"))
    ...
    def _show_missing_files_warning(self, verification) -> None:
        ...
        Dialogs.warning(
            self,
            self.localizer.get_string("step4_verify_files_missing_title"),
            f"{self.localizer.get_string('step4_verify_files_missing_message')}\n\n{missing}",
        )
    ...
    def check_global_datasets_status(self, show_feedback: bool = False) -> None:
        ...
        if not missing:
            msg = self.localizer.get_string(
                "step4_status_global_data_all_present",
            ...
            if self.global_toggle_btn:
                self.global_toggle_btn.setText(escape_mnemonic(f"▶ {self.localizer.get_string('step4_accordion_global_full', default='1. Globale Geodaten (POP & GADM) [✓ 5/5 vorhanden]')}"))
            if self.global_container:
                self.global_container.hide()
            if self.pbf_container:
                self.pbf_container.show()
            if self.pbf_toggle_btn:
                self.pbf_toggle_btn.setText(escape_mnemonic(f"▼ {self.localizer.get_string('step4_accordion_osm', default='2. OSM-Regionen auswählen & herunterladen (Geofabrik)')}"))
            if show_feedback:
                Dialogs.info(self, self.localizer.get_string("step4_dialog_global_status_title", default="POP & GADM Status"), msg)
        else:
            ...
            msg = self.localizer.get_string(
                "step4_status_global_data_missing",
                missing=missing_str,
            ...
            if self.global_status_label:
                self.global_status_label.setText(f"Status: {msg}")
            if self.global_toggle_btn:
                self.global_toggle_btn.setText(escape_mnemonic(f"▼ {self.localizer.get_string('step4_accordion_global_partial', present_count=present_count, default=f'1. Globale Geodaten (POP & GADM) [{present_count}/5 vorhanden]')}"))
            if self.global_container:
                self.global_container.show()
            if show_feedback:
                Dialogs.warning(self, self.localizer.get_string("step4_dialog_global_status_title", default="POP & GADM Status"), msg)
        self._update_next_button_state()
    ...
    def start_global_download(self) -> None:
        ...
        if self.global_status_label:
            self.global_status_label.setText(self.localizer.get_string("step4_status_preparing_download", default="Download von POP & GADM wird gestartet..."))
    ...
    def _handle_global_download_finished(self, success: bool, message: str) -> None:
        ...
        if success:
            ...
            if self.global_file_progress_label:
                self.global_file_progress_label.setText("")
            if self.global_overall_progress_label:
                self.global_overall_progress_label.setText(self.localizer.get_string("step4_status_all_downloads_complete", default="Download abgeschlossen."))
            if self.global_status_label:
                self.global_status_label.setText(self.localizer.get_string("step4_status_global_complete", default="Download abgeschlossen: Alle POP & GADM Daten vorhanden."))
            Dialogs.info(self, self.localizer.get_string("step4_dialog_global_download_title", default="Download POP & GADM"), message)
        else:
            if self.global_status_label:
                self.global_status_label.setText(self.localizer.get_string("step4_status_download_finished_msg", message=message, default=f"Download beendet: {message}"))
            Dialogs.warning(self, self.localizer.get_string("step4_dialog_global_download_title", default="Download POP & GADM"), message)
        self.check_global_datasets_status(show_feedback=False)
    ...
    def stop_global_download(self) -> None:
        if self._global_download_worker:
            self._global_download_worker.stop()
            if self.global_status_label:
                self.global_status_label.setText(self.localizer.get_string("step4_status_cancelling", default="Breche Download ab..."))
    ...
    def check_geofabrik_index_freshness(self) -> None:
        ...
        if not info["exists"]:
            if self.index_status_label:
                self.index_status_label.setText(self.localizer.get_string("step4_status_index_missing", default="Geofabrik-Index: Nicht vorhanden"))
            ...
            if self.index_status_label:
                self.index_status_label.setText(
                    self.localizer.get_string("step4_status_index_outdated", date=mtime_str, default=f"Geofabrik-Index: Veraltet (Stand: {mtime_str})")
                )
            self.update_geofabrik_index()
        else:
            mtime_str = info.get("mtime_str", "")
            if self.index_status_label:
                self.index_status_label.setText(
                    self.localizer.get_string("step4_status_index_up_to_date", date=mtime_str, default=f"Geofabrik-Index: Aktuell (Stand: {mtime_str})")
                )
    ...
    def update_geofabrik_index(self) -> None:
        ...
        if self.index_status_label:
            self.index_status_label.setText(self.localizer.get_string("step4_status_index_downloading", default="Geofabrik-Index wird heruntergeladen..."))
