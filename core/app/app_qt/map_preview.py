...
def create_satellite_layer(name: str = "Satellite") -> Optional[QgsRasterLayer]:
    """Creates a high quality Esri World Imagery satellite raster layer with bilinear resampling."""
    if QgsRasterLayer is None:
        return None
    encoded_url = quote(SATELLITE_TILE_URL, safe=":/")
    encoded_ua = quote(APP_USER_AGENT)
    uri = f"type=xyz&url={encoded_url}&zmax=19&zmin=0&crs=EPSG3857&http-header:User-Agent={encoded_ua}"
    layer = QgsRasterLayer(uri, name, "wms")
    if layer.isValid():
...
def setup_map_canvas(canvas: QgsMapCanvas, crs_authid: str = "EPSG:3857") -> None:
    ...
    if hasattr(canvas, "setDestinationCrs") and QgsCoordinateReferenceSystem is not None:
        canvas.setDestinationCrs(QgsCoordinateReferenceSystem(crs_authid))
...
from core.locales import localizer as default_localizer


class OsmAttributionOverlay(QtWidgets.QLabel if QtWidgets is not None else object):
    """Clickable attribution overlay for QGIS map canvases supporting OSM and Satellite modes."""
    ...
    def __init__(self, parent_canvas: Optional[object], localizer=None):
        ...
        self.localizer = localizer or getattr(parent_canvas, "localizer", None) or default_localizer
        self._mode = "osm"
        self.update_text()
        self.setStyleSheet(
            "QLabel {"
    ...
    def update_text(self, localizer=None):
        if localizer is not None:
            self.localizer = localizer
        loc = self.localizer or default_localizer
        if self._mode == "satellite":
            sat_text = loc.get_string("map_copyright_satellite", default="© Esri, Maxar, Earthstar Geographics") if loc else "© Esri, Maxar, Earthstar Geographics"
            self.setText(f'<a href="https://www.esri.com" style="color: #0055aa; text-decoration: none;">{sat_text}</a>')
        else:
            label_text = loc.get_string("osm_copyright", default="© OpenStreetMap-Mitwirkende") if loc else "© OpenStreetMap-Mitwirkende"
...
class MapPreviewWidget(_Q_WIDGET_BASE):
    ...
    def _build_ui(self) -> None:
        ...
        self.attribution_overlay = OsmAttributionOverlay(self.canvas, localizer=getattr(self, "localizer", None))

    def retranslate_ui(self, localizer=None) -> None:
        if hasattr(self, "attribution_overlay") and self.attribution_overlay:
            self.attribution_overlay.update_text(localizer)
