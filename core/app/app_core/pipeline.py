...
class PipelineCallbacks:
    ...
    file_locked: Callable[[str], bool] = lambda _path: False


_OSM_PROGRESS_LABELS = {
    "merge": "PBF-Dateien zusammenführen",
    "pa_extract": "Untersuchungsgebiet ausschneiden",
...
def resolve_all_iso3_country_codes(
    ...
    def _add_alpha2(a2: Any) -> None:
        if not a2 or not isinstance(a2, str):
            return
        a2 = a2.strip().upper()
        if a2 in CUSTOM_GADM_ISO3_MAP:
            code3 = CUSTOM_GADM_ISO3_MAP[a2]
...
class UrbanActPipeline:
    ...
    def run_phase_a_only(
        ...
        *,
        ...
        if reused is not None:
            callbacks.phase_started("OSM-Vorverarbeitung (wiederverwendet)", 1, 9)
            callbacks.phase_progress(100)
            callbacks.log("Vorhandene OSM-Vorverarbeitungsdateien werden wiederverwendet.")
        ...
        callbacks.phase_started("OSM-Vorverarbeitung", 1, 9)
        update_manifest_phase(
            manifest,
            "osm_phase_a",
            "running",
    ...
    def run(
        ...
        *,
        ...
        run_nationwide_intensity: bool = False,
        nationwide_parameters: Optional[Mapping[str, object]] = None,
    ) -> ModelPipelineResult:
        stop_event = stop_event or Event()
        paths = canonical_project_paths(self.project_path)
        manifest = load_pipeline_manifest(self.project_path)
        ...
        (project_input / "Nationwide_params.json").write_text(
            json.dumps({
                "enabled": bool(run_nationwide_intensity),
                **(dict(nationwide_parameters or {})),
            }, indent=2),
            encoding="utf-8",
        )
        pbfs = self._pbf_paths()
        ...
        if reused_phase_a is not None:
            ...
            callbacks.phase_started("OSM-Vorverarbeitung (wiederverwendet)", 1, 9)
            callbacks.phase_progress(100)
            log("Gültige OSM-Vorverarbeitungs-Outputs werden wiederverwendet.")
        else:
            update_manifest_phase(
                manifest,
                "osm_phase_a",
        ...
        def model_started(description, index: int, _total: int) -> None:
            ...
            phase_num = index + 1 if index < 6 else index + 2
            callbacks.phase_started(description.name, phase_num, 9)
            callbacks.phase_progress(0)
            log(f"Starte QGIS-Schritt {index}/{len(MODEL_DESCRIPTIONS)}: {description.key} ({description.name}) ...")
            update_manifest_phase(
                manifest,
                description.key,
                "running",
        ...
        def model_finished(description, _raw_outputs, model_context) -> None:
            ...
            if description.key == "model4":
                next_step_desc = "OSM-Verkehrsnetz- und POI-Aufbereitung (Phase C)"
            elif idx >= 0 and idx + 1 < len(MODEL_DESCRIPTIONS):
            ...
            if description.key == "model5" and run_nationwide_intensity:
                try:
                    callbacks.phase_started("Modell 5 (Landesweit) – Innergemeindliche Zentralitäten", 8, 9)
                    log("Starte Modell 5 (Landesweit) – Innergemeindliche Zentralitätsschätzung ...")
                    update_manifest_phase(manifest, "nationwide_intensity", "running")
                    save_pipeline_manifest(self.project_path, manifest)

                    from core.scripts.qgis.scripts.Model5_NationwideIntensity import run_nationwide_intensity_estimation

                    nw_params = dict(nationwide_parameters or {})
                    radius_val = float(nw_params.get("radius", 500.0))
                    min_int_val = float(nw_params.get("min_intensity", 7.0))
                    min_int_l3_val = float(nw_params.get("min_intensity_level_3", 10.0))

                    seeds_path = Path(self.project_path) / "processed" / "qgis_output" / "model2_ZoneClass" / "central_place_points.gpkg"
                    output_dir = Path(self.project_path) / "processed" / "qgis_output" / "model5_NationwideIntensity"
                    output_path = output_dir / "inner_urban_central_points_nationwide.gpkg"

                    styles_dir_path = os.path.join(tool_root(), "core", "scripts", "qgis", "styles")
                    res_path = run_nationwide_intensity_estimation(
                        seeds_gpkg=seeds_path,
                        pois_or_pbf=Path(phase_a["merged_pbf"]),
                        output_gpkg=output_path,
                        poi_points_path=phase_a.get("study_area_points"),
                        poi_polygons_path=phase_a.get("study_area_polygons"),
                        local_crs=prepared.local_crs,
                        radius=radius_val,
                        min_intensity=min_int_val,
                        min_intensity_level_3=min_int_l3_val,
                        osmium_exe=runtime.executable,
                        styles_dir=styles_dir_path,
                        log=log,
                        progress=callbacks.phase_progress,
                    )

                    update_manifest_phase(
                        manifest,
                        "nationwide_intensity",
                        "done",
                        outputs={
                            "inner_urban_central_points_nationwide": str(res_path),
                            "central_place_points_nationwide": str(output_dir / "central_place_points_nationwide.gpkg"),
                            "sector_all_intensity": str(output_dir / "sector_all_intensity.tif"),
                            "sector_all_points": str(output_dir / "sector_all_points.gpkg"),
                        },
                    )
                    save_pipeline_manifest(self.project_path, manifest)
                    callbacks.output("inner_urban_central_points_nationwide", str(res_path))
                    callbacks.output("sector_all_intensity_nationwide", str(output_dir / "sector_all_intensity.tif"))
                    log(f"Modell 5 (Landesweit) – Innergemeindliche Zentralitätsschätzung erfolgreich abgeschlossen: {res_path.name}")
                except Exception as exc:
                    log(f"Warnung bei landesweiter Intensitätsschätzung: {exc}")
                    update_manifest_phase(manifest, "nationwide_intensity", "failed", error=str(exc))
                    save_pipeline_manifest(self.project_path, manifest)
        ...
        def phase_c(model_context: dict[str, object]) -> Mapping[str, object]:
            ...
            if phase_c_reusable:
                ...
                callbacks.phase_started("OSM-Verkehrsnetz & POIs (wiederverwendet)", 7, 9)
                callbacks.phase_progress(100)
                log("OSM-Verkehrsnetz & POIs: vorhandene, vollständige Outputs werden wiederverwendet.")
                for key, value in outputs.items():
                    callbacks.output(key, str(value))
                return {
            ...
            callbacks.phase_started("OSM-Verkehrsnetz & POIs", 7, 9)
        ...
        try:
            result = pipeline.run(
                ...
                on_file_locked=callbacks.file_locked,
            )
        except Exception as exc:
    ...
    try:
        ...
        if layer.isValid():
            ...
            if layer.crs() != dest_crs:
                ...
                write_fn = getattr(QgsVectorFileWriter, "writeAsVectorFormatV3", None) or getattr(QgsVectorFileWriter, "writeAsVectorFormatV2", None) or QgsVectorFileWriter.writeAsVectorFormat
                writer_res = write_fn(
...
def post_process_exports_and_styles(project_path: str, *, log: Optional[Callable[[str], None]] = None) -> None:
    ...
    # 2.b.3 Model 5 Nationwide Intensity styles
    model5_nationwide_dir = os.path.join(project_path, "processed", "qgis_output", "model5_NationwideIntensity")
    if os.path.isdir(model5_nationwide_dir) and os.path.isfile(point_style_src):
        for target_name in ("nationwide_centrality_points.qml", "central_place_points_nationwide.qml"):
            target_path = os.path.join(model5_nationwide_dir, target_name)
            try:
                shutil.copy2(point_style_src, target_path)
                if log:
                    log(f"QML-Style kopiert: {target_path}")
                ...
                if log:
                    log(f"Warnung beim Kopieren von Nationwide-Punkt-Style: {e}")
    ...
    # Register temporary QGIS processing folder cleanup on application exit only (never during runtime!)
    _register_atexit_temp_cleanup()


_cleanup_registered = False


def _register_atexit_temp_cleanup() -> None:
    ...
    def _cleanup_on_exit():
        try:
            ...
            for p in temp_dir.glob("processing_*"):
                if p.is_dir():
                    try:
