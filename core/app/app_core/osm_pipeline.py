...
def _paths(project_path: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "study_area_points": osm / "03_features" / "model5_pois_study_area_cleaned.gpkg",
        "study_area_polygons": osm / "03_features" / "poi_polygons.gpkg",
        "model5_pois_nationwide_all": osm / "03_features" / "model5_pois_nationwide_all.gpkg",
        "model5_pois_nationwide_cleaned": osm / "03_features" / "model5_pois_nationwide_cleaned.gpkg",
        "model5_pois_study_area_cleaned": osm / "03_features" / "model5_pois_study_area_cleaned.gpkg",
...
class OsmPipeline:
    ...
    def run_phase_a(
        ...
        *,
        ...
        self.log("[OSM-Vorverarbeitung 1/4] Starte PBF-Dateien zusammenführen ...")
        ...
        if len(inputs) == 1:
            ...
            try:
                ...
                self._check_cancelled()
                self.log("OSM-Vorverarbeitung: Überlappende OSM-Objekte konsolidieren ...")
                self._run(
                    [
                        "time-filter",
                        merged_versions,
        ...
        self.log("[OSM-Vorverarbeitung 1/4] PBF-Zusammenführung abgeschlossen. Nächster Schritt: Untersuchungsgebiet (PA/IA1) ausschneiden (Schritt 2/4) ...")

        self.progress("pa_extract", 1, total)
        self.log("[OSM-Vorverarbeitung 2/4] Starte Untersuchungsgebiet (PA & IA1) ausschneiden ...")
        self._check_cancelled()
        self._run(
            [
                "extract",
        ...
        self.log("[OSM-Vorverarbeitung 2/4] Untersuchungsgebiet-Ausschnitt erstellt. Nächster Schritt: Städte und Gemeinden extrahieren (Schritt 3/4) ...")
        ...
        self.log("[OSM-Vorverarbeitung 3/4] Starte Städte und Gemeinden extrahieren ...")
        self._check_cancelled()
        export_cities_fn(
        ...
        self.log("[OSM-Vorverarbeitung 3/4] Städte und Gemeinden extrahiert. Nächster Schritt: Unbewohnte Flächen filtern (Schritt 4/4) ...")

        self.progress("uninhabited", 3, total)
        self.log("[OSM-Vorverarbeitung 4/4] Starte nachweislich unbewohnte Flächen extrahieren ...")
        self._check_cancelled()
        ...
        for required in ("merged_pbf", "osm_cities", "osm_pop_0"):
            if not paths[required].is_file():
                raise RuntimeError(f"OSM-Vorverarbeitung lieferte den erwarteten Output nicht: {paths[required]}")
        self.progress("complete", total, total)
        self.log("[OSM-Vorverarbeitung] Alle 4 Teilschritte der OSM-Vorverarbeitung erfolgreich abgeschlossen. Nächster Schritt: GADM-Zuschnitt und QGIS Modell 1 (Datenaufbereitung) ...")
        ...
        *,
        ...
        for source in (config.merged_pbf, config.pa_ia1_poly, config.study_area_poly, config.master_network):
            if not Path(source).is_file():
                raise FileNotFoundError(f"OSM-Verkehrsnetz-Input fehlt: {source}")
        ...
        self.log("[OSM-Verkehrsnetz & POIs 1/2] Starte hierarchisches Straßennetz generieren (export_network) ...")
        self._check_cancelled()
        ...
        self.log("[OSM-Verkehrsnetz & POIs 1/2] Hierarchisches Straßennetz generiert. Nächster Schritt: POIs und Flächen der Study-Area extrahieren (Schritt 2/2) ...")

        self.progress("study_area_prepare", 11, phase_c_steps)
        self.log("[OSM-Verkehrsnetz & POIs 2/2] Starte POIs und Flächen der Study-Area extrahieren (export_study_area) ...")
        self._check_cancelled()
        ...
        self.log("[OSM-Verkehrsnetz & POIs] Straßennetz- und POI-Aufbereitung vollständig abgeschlossen. Nächster Schritt: QGIS Modell 5 (Urbane Zentralität) ...")
        required = ("network_original", "network_modified", "study_area_points", "study_area_polygons")
        for key in required:
            if not paths[key].is_file():
                raise RuntimeError(f"OSM-Verkehrsnetz- und POI-Aufbereitung lieferte den erwarteten Output nicht: {paths[key]}")
