...
MODEL_DESCRIPTIONS: tuple[ModelDescription, ...] = (
    ModelDescription(
        ...
        (
            ...
            _output("pop_raster_corr_areas", "pop_raster_corr_areas", "pop_raster_corr_areas.gpkg"),
        ),
    ),
    ModelDescription(
    ...
    ModelDescription(
        ...
        (
            ...
            _binding("local_crs", "local_crs"),
            _binding("poi_points", "osm_points"), _binding("poi_polygons", "osm_polygons"),
        ...
        *,
        ...
        try:
            for index, description in enumerate(self.descriptions, 1):
                ...
                if description.key == "model5" and phase_c_hook:
                ...
                for _out, _act, fixed_path in output_bindings:
                    if os.path.exists(fixed_path):
                        try:
                            ...
                            if on_log:
                                on_log(f"Hinweis: Konnte vorherige Ausgabedatei {fixed_path} nicht löschen: {exc}")
