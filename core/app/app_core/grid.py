...
def transfer_selected_cells(
    old_grid_map_data: Optional[list[dict]],
    new_grid_map_data: list[dict],
) -> dict[str, set[int]]:
    """Transfer assigned area selections from an old grid to a newly generated/shifted grid.

    Uses relative coordinate mapping relative to center (0,0) so the chosen pattern remains
    anchored to the project center when moving position or changing radius, and falls back to
    spatial polygon overlap if relative coordinates are unavailable or cell sizes differ.
    """
    selected_cells = create_selected_cells()
    if not old_grid_map_data or not new_grid_map_data:
        return selected_cells

    # Relative coordinate matching (anchored to center)
    old_rel_map = {
        cell["rel_coord"]: cell["area_type"]
        for cell in old_grid_map_data
        if cell.get("area_type") and "rel_coord" in cell
    }

    if old_rel_map:
        matched_any = False
        for new_cell in new_grid_map_data:
            rel_coord = new_cell.get("rel_coord")
            if rel_coord in old_rel_map:
                area_type = old_rel_map[rel_coord]
                if area_type in selected_cells:
                    new_cell["area_type"] = area_type
                    selected_cells[area_type].add(new_cell["id"])
                    matched_any = True
        if matched_any:
            return selected_cells

    # Fallback: spatial polygon overlap matching
    old_selected = [
        cell for cell in old_grid_map_data
        if cell.get("area_type") and cell.get("shapely_poly_wgs84")
    ]
    if not old_selected:
        return selected_cells

    for new_cell in new_grid_map_data:
        new_poly = new_cell.get("shapely_poly_wgs84")
        if not new_poly:
            continue
        ...
        for old_cell in old_selected:
            old_poly = old_cell["shapely_poly_wgs84"]
            if not old_poly:
                continue
            if centroid.within(old_poly):
                best_mode = old_cell["area_type"]
                break
            try:
                if new_poly.intersects(old_poly):
                    overlap = new_poly.intersection(old_poly).area
        ...
        if best_mode and best_mode in selected_cells:
            new_cell["area_type"] = best_mode
            selected_cells[best_mode].add(new_cell["id"])
...
def restore_grid_geometries(grid_map_data: list[dict]) -> list[dict]:
    ...
    for cell in grid_map_data:
        ...
        if restored is None:
            lon_lat = []
            for item in cell.get("wgs84_coords_map", []):
                if isinstance(item, (list, tuple)) and len(item) == 2:
                    lon_lat.append((item[1], item[0]))
                else:
                    lon_lat = []
            if len(lon_lat) >= 3:
...
def generate_grid_map_data(center_lat: float, center_lon: float, cell_size_m: int, radius_km: int | float) -> list[dict]:
    ...
    for i in range(len(x_coords) - 1):
        for j in range(len(y_coords) - 1):
            ...
            cell_center_x = xs_utm + half_cell
            cell_center_y = ys_utm + half_cell
            if math.hypot(cell_center_x - center_x, cell_center_y - center_y) > radius_m:
                continue

            rel_x = int(round((cell_center_x - center_x) / cs))
            rel_y = int(round((cell_center_y - center_y) / cs))
