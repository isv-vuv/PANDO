...
def _ensure_gdal_environment() -> None:
    ...
    cpus = str(os.cpu_count() or 8)
    os.environ["GDAL_NUM_THREADS"] = "ALL_CPUS"
    os.environ["GDAL_CONCURRENCY"] = "ALL_CPUS"
...
def run_qgis_model_processing(
    ...
    *,
    ...
    max_retries = 3
    result = None
    # Attach PyQGIS Message Log hook & configure max threads
    try:
        from qgis.core import QgsApplication
        qgis_app = QgsApplication.instance()
        if qgis_app:
            if hasattr(qgis_app, "setMaxThreads"):
                qgis_app.setMaxThreads(os.cpu_count() or 8)
            msg_log = qgis_app.messageLog()
            if msg_log and hasattr(msg_log, "messageReceived"):
                def _qgis_msg_handler(msg, tag, level):
                    if on_log and str(msg).strip():
                        on_log(f"[QGIS-{tag}] {str(msg).strip()}")
                msg_log.messageReceived.connect(_qgis_msg_handler)
    except Exception:
        pass
