...
All .poly files will be in WGS84 (EPSG:4326) coordinates as required by OSM tools.''')
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for current, feature in enumerate(features):
            ...
            # Validate that there is at least one non-empty ring
            has_valid_rings = any(any(len(ring) >= 3 for ring in poly) for poly in polygons if poly)
            if not has_valid_rings:
                feedback.reportError(f'Feature ID {feature.id()} contains no valid polygon rings with at least 3 vertices')
                continue
