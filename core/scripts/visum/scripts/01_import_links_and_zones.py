...
def _format_geom_wkt(geom) -> str:
    """Konvertiert Geometrien in WKT und vereinfacht komplexe Küsten-/Insel-Polygone für Visum."""
    if geom is None or geom.is_empty:
        return ""
    try:
        if hasattr(geom, "simplify"):
            geom = geom.simplify(tolerance=2.0, preserve_topology=True)
    except Exception:
        pass
    return geom.wkt if geom and not geom.is_empty else ""
...
def prepare_geopackage_df(gdf: gpd.GeoDataFrame, is_mainzone: bool = False) -> pd.DataFrame:
    ...
    if df.geometry is not None:
        df["WKTSURFACE"] = df.geometry.apply(_format_geom_wkt)
...
"""
    ...
    # Wichtig: WKTSURFACE nicht in die .net-Datei schreiben, da riesige WKTs (z. B. Inseln/Küsten) Visums .net-Parser einfrieren lassen.
    # Stattdessen werden die Flächengeometrien direkt danach performant via SetMultiAttValues zugewiesen.
    df_mainzones_net = (
        df_mainzones.drop(columns=["WKTSURFACE"], errors="ignore")
        if df_mainzones is not None
        else None
    )
    df_zones_net = df_zones.drop(columns=["WKTSURFACE"], errors="ignore")

    with open(output_net_path, "w", encoding="utf-8-sig") as f:
        f.write(header)
        f.write(uda_block)

        if df_mainzones_net is not None:
            f.write("* \n* Tabelle: Hauptbezirke\n* \n$MAINZONE:" + ";".join(df_mainzones_net.columns) + "\n")
            f.write(df_mainzones_net.to_csv(sep=";", index=False, header=False, na_rep=""))
            f.write("\n")

        f.write("* \n* Tabelle: Bezirke\n* \n$ZONE:" + ";".join(df_zones_net.columns) + "\n")
        f.write(df_zones_net.to_csv(sep=";", index=False, header=False, na_rep=""))
...
def run_import_links_and_zones(
    ...
    try:
        ...
        visum = step7.ensure_visum_language(visum, app_lang, log=logging.info)
    except Exception:
        pass
    ...
    if "WKTSURFACE" in df_zones.columns:
        try:
            ...
            visum_zone_ids["VISUM_ID"] = pd.to_numeric(visum_zone_ids["VISUM_ID"], errors="coerce")
            visum_zone_ids["NO"] = pd.to_numeric(visum_zone_ids["NO"], errors="coerce")

            df_zones_copy = df_zones.copy()
            df_zones_copy["NO"] = pd.to_numeric(df_zones_copy["NO"], errors="coerce")

            df_zones_merged = pd.merge(df_zones_copy, visum_zone_ids.dropna(), on="NO")
            valid_zone_wkts = df_zones_merged[df_zones_merged["WKTSURFACE"].astype(str).str.len() > 5]
            zone_wkts = valid_zone_wkts[["VISUM_ID", "WKTSURFACE"]].dropna().values.tolist()
            ...
            logging.warning(f"Konnte WKTSURFACE für Bezirke nicht zuweisen: {e}")
    ...
    if df_mainzones is not None and "WKTSURFACE" in df_mainzones.columns:
        try:
            ...
            visum_mainzone_ids["VISUM_ID"] = pd.to_numeric(visum_mainzone_ids["VISUM_ID"], errors="coerce")
            visum_mainzone_ids["NO"] = pd.to_numeric(visum_mainzone_ids["NO"], errors="coerce")

            df_mainzones_copy = df_mainzones.copy()
            df_mainzones_copy["NO"] = pd.to_numeric(df_mainzones_copy["NO"], errors="coerce")

            df_mainzones_merged = pd.merge(df_mainzones_copy, visum_mainzone_ids.dropna(), on="NO")
            valid_mainzone_wkts = df_mainzones_merged[df_mainzones_merged["WKTSURFACE"].astype(str).str.len() > 5]
            mainzone_wkts = valid_mainzone_wkts[["VISUM_ID", "WKTSURFACE"]].dropna().values.tolist()

            if mainzone_wkts:
                visum.Net.MainZones.SetMultiAttValues("WKTSURFACE", mainzone_wkts)
                logging.info(f"{len(mainzone_wkts)} Polygon-Flächengeometrien erfolgreich den Hauptbezirken zugewiesen.")
        except Exception as e:
            logging.warning(f"Konnte WKTSURFACE für Hauptbezirke nicht zuweisen: {e}")
