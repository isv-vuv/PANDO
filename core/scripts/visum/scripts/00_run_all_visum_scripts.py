...
def disconnect_active_visum(close_app: bool = False, log=print) -> None:
    """Safely disconnects and releases any active PTV Visum COM instance."""
    global _ACTIVE_VISUM_INSTANCE
    if _ACTIVE_VISUM_INSTANCE is not None:
        try:
            if close_app:
                try:
                    _ACTIVE_VISUM_INSTANCE.Exit()
                except Exception:
                    pass
        except Exception:
            pass
            ...
            try:
                ...
                pass
            if log:
                log("PTV Visum COM-Verbindung wurde getrennt und Ressourcen freigegeben.")
...
def run_visum_postprocessing_chain(project_path: str | Path, *, start_step: int = 1, log=print) -> bool:
    ...
    try:
        ...
        if start_step <= 1:
            log("Schritt 1/7: Strecken und Verkehrszellen werden importiert")
