...
def calculate_zone_distances(visum, nf, use_island_logic: bool = False):
    ...
    if use_island_logic:
        ...
        logging.info("Starte REINE DISTANZ-Berechnung für Bezirke zu Zugangsknoten (Typen 1 bis 4)...")
...
def run_generate_connectors(target_project_dir=None, visum=None):
    ...
    def create_connectors_for_type(anb_typeno, filter_zentralitaet, anb_knoten_typnr, max_entfernung, anz_anb,
        ...
        for nt in node_types_to_try:
            ...
            zone_types = visum.Net.Zones.GetMultipleAttributes(["TYPENO"], OnlyActive=True)
            zentralitaeten = sorted(set(z[0] for z in zone_types))
            logging.warning(
                f'{visum.Net.Zones.CountActive} Bezirk(e) (Zentralitäten: {zentralitaeten}) konnten nicht an ZK_TYP '
                f'{anb_knoten_typnr} in {max_entfernung} m Anbindung finden. Versuche Stufe {nt}...'
            )
        ...
        # 3. Notfall-Attempt: Radius auf 500 km öffnen & ZK_TYP <= 5 freigeben
        if visum.Net.Zones.CountActive > 0:
            zone_types = visum.Net.Zones.GetMultipleAttributes(["TYPENO"], OnlyActive=True)
            zentralitaeten = sorted(set(z[0] for z in zone_types))
            logging.warning(
                f'Finaler Anbindungsversuch für {visum.Net.Zones.CountActive} Bezirk(e) (Zentralitäten: {zentralitaeten}): Öffne Notfall-Radius auf 500 km und ZK_TYP <= 5...'
            )
            ...
            visum.Net.CreateODConnectors(0, 500000, anz_anb, anz_anb, anb_typeno)
