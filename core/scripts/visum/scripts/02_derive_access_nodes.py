...
class AccessNodeDeriver:
    ...
    def set_open_end_nodes(self):
        """Setzt Knotentypen für offene Enden von Strecken der Klasse 1 (Autobahnen)."""
        logging.info("Setze Knotentypen für offene Enden...")
        self.lf.Init()
        self.lf.AddCondition("OP_NONE", False, "ZK_OBERTYP_UNGERICHTET", "EqualVal", 1)
        self.nf.Init()
    ...
    def calculate_reachable_nodes(self, max_start_nodes: int = 10):
        """Ermittelt erreichbare Knoten über Isochronen für mehrere Startknoten."""
        logging.info(f"Berechne 'Reachable_Node' Attribut via Isochronen für bis zu {max_start_nodes} Startknoten...")
        ...
        # 1. Startknoten aus dem Planungsraum (Planning Area / Study Area / Bezirkszentren)
        try:
            map_matcher = self.Visum.Net.CreateMapMatcher()
            self.Visum.Filters.InitAll()
            zf = self.Visum.Filters.ZoneFilter()
            zf.Init()
            if self.Visum.Net.Zones.AttrExists("ZONETYPE"):
                zf.AddCondition("OP_NONE", False, "ZONETYPE", "EqualVal", "PA")
            if self.Visum.Net.Zones.CountActive == 0:
                zf.Init()
                if self.Visum.Net.Zones.AttrExists("TYPENO"):
                    zf.AddCondition("OP_NONE", False, "TYPENO", "LessEqualVal", 2)
            if self.Visum.Net.Zones.CountActive == 0:
                zf.Init()

            zone_coords = self.Visum.Net.Zones.GetMultipleAttributes(["XCOORD", "YCOORD"], OnlyActive=True)
            zf.Init()

            if zone_coords:
                step = max(1, len(zone_coords) // 5)
                for xc, yc in zone_coords[::step][:5]:
                    res = map_matcher.GetNearestNode(xc, yc, 50000, True)
                    if res.Success and res.Node:
                        n_no = int(res.Node.AttValue("NO"))
                        if n_no not in start_node_nos:
                            start_node_nos.append(n_no)
        except Exception as e:
            logging.debug(f"Planungsraum-Startknoten-Suche: {e}")

        # 2. Startknoten aus übergeordnetem Hauptstraßennetz (Fähren ausschließen)
        try:
            self.Visum.Filters.InitAll()
            self.nf.Init()
            if self.Visum.Net.Nodes.AttrExists("Ferry_Origin"):
                self.nf.AddCondition("OP_NONE", False, "Ferry_Origin", "EqualVal", False)
            if self.Visum.Net.Nodes.AttrExists("ZK_TYP_DETAIL"):
                self.nf.AddCondition("OP_AND", False, "ZK_TYP_DETAIL", "NotEqualVal", "N-N")
                self.nf.AddCondition("OP_AND", False, "ZK_TYP_DETAIL", "NotEqualVal", "")

            cand_nodes = self.Visum.Net.Nodes.GetMultipleAttributes(["NO", "COUNT:INLINKS"], OnlyActive=True)
            self.nf.Init()

            if cand_nodes:
                sorted_cands = sorted(cand_nodes, key=lambda x: x[1] if x[1] is not None else 0, reverse=True)
                for n in sorted_cands:
                    n_no = int(n[0])
                    if n_no not in start_node_nos:
                        start_node_nos.append(n_no)
                    if len(start_node_nos) >= max_start_nodes:
                        break
        except Exception as e:
            logging.debug(f"Hauptnetz-Startknoten-Suche: {e}")

        # Fallback falls noch nicht genug Startknoten gefunden
        if len(start_node_nos) < max_start_nodes:
            all_nodes = self.Visum.Net.Nodes.GetMultipleAttributes(["NO", "COUNT:INLINKS"], OnlyActive=False)
            if all_nodes:
                sorted_all = sorted(all_nodes, key=lambda x: x[1] if x[1] is not None else 0, reverse=True)
                for n in sorted_all:
                    n_no = int(n[0])
                    if n_no not in start_node_nos:
                        start_node_nos.append(n_no)
        ...
        logging.info(f"Ausgewählte Isochronen-Startknoten ({len(start_node_nos)}): {start_node_nos}")
        ...
        total_nodes_count = len(nodes_isoctimes_df)
        valid_start_nodes_count = 0
        ...
        for node_no in start_node_nos:
            try:
                ...
                reached_count = int(accessible_from_this.sum())
                reach_share = reached_count / max(1, total_nodes_count)

                # Isolierte Startknoten (z. B. Inseln mit < 15 % Netzabdeckung) verwerfen
                if reach_share < 0.15:
                    logging.info(
                        f"Startknoten {node_no} erreicht nur {reached_count} Knoten ({reach_share:.1%}) und liegt isoliert – überspringe."
                    )
                    continue

                valid_start_nodes_count += 1
                logging.info(f"Hauptnetz-Startknoten {node_no} erreicht {reached_count} Knoten ({reach_share:.1%}).")

                # Ein Knoten muss von ALLEN validen Hauptnetz-Startknoten erreichbar sein (UND)
                if valid_start_nodes_count == 1:
                    nodes_isoctimes_df['Accessible'] = accessible_from_this
                else:
        ...
        if valid_start_nodes_count == 0:
            logging.warning("Kein übergeordneter Hauptnetz-Startknoten gefunden. Setze Accessible auf True.")
            nodes_isoctimes_df['Accessible'] = True
